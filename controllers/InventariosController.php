...
class InventariosController {
    ...
    public static function editar(Router $router) {
        ...
        // ✅ OBTENER DETALLE DEL MOVIMIENTO (AJAX GET)
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'obtenerDetalle') {
            $idMovimiento = $_GET['id'] ?? 0;

            // Llamada al procedimiento para traer el detalle
            $detalle = InventarioDetalle::procedureLista('prc_DetalleMovimientoInventario', [$idMovimiento]);
       
            // Renderizamos el contenido del modal (solo el HTML del detalle)
            ob_start();
            include __DIR__ . '../../views/admin/gestion/logistica/inventario/detalleinventario.php';
            $html = ob_get_clean();
            echo $html;
            exit;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'editarDetalle') {
            $id = $_POST['id_detalle'];
            ...
            try {
                ...
                $inventario::procedureMantenimiento("prm_detalle_inventario_editar", [$id,$cantidad,$costo,$usuario]);
        ...
        // ✅ Si se entra directo por URL, redirigir a la lista
        header('Location: /admin/gestion/logistica/inventario');
    }
