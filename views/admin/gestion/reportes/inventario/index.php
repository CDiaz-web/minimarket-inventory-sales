    ...
    <div class="table-header">
        <div class="table-actions">
            ...
            <button class="boton boton--success-light"                  
                ...
                data-table="tabla"
                data-file="inventario.xlsx"
                data-sheet="Inventario"
            >
                Exportar xls
            </button>
            <form method="POST" action="/admin/gestion/reportes/pdfinventario" target="_blank">
        ...
        <div class="table-actions">
            ...
            <form method="GET" action="/admin/gestion/reportes/inventario" class="dashboard__filtros">      
                ...
                <tbody class="table-tbody" id="tabla">
                    <?php foreach($ventas as $row) { ?>
                        <tr class="table__tr">
                            <td><?= $row->codigo ?></td>
                            <td><?= $row->producto ?></td>
                            <td><?= number_format($row->stock_actual,2) ?></td>
                            <td><?= number_format($row->stock_comprometido,2) ?></td>
                            <td><?= number_format($row->stock_disponible,2) ?></td>
                            <td><?= number_format($row->stock_min,2) ?></td>
                            <td><?= number_format($row->stock_max,2) ?></td>
                            <td><?= $row->estado ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
