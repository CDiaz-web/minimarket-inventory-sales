    ...
    <div class="table-header">
        <div class="table-actions">
            ...
            <button class="boton boton--success-light"                  
                ...
                data-table="tabla"
                data-file="vetascliente.xlsx"
                data-sheet="Ventas Cliente"
            >
                Exportar xls
            </button>
            <form method="POST" action="/admin/gestion/reportes/pdfcliente" target="_blank">
        ...
        <div class="table-actions">
            ...
            <form method="GET" action="/admin/gestion/reportes/cliente" class="dashboard__filtros">      
        ...
        <?php if(!empty($venta_clientes)) { ?>
            <table id="tabla"  class="table" data-table data-page-size="10">
                ...
                <tbody class="table-tbody" id="tabla">
                    <?php foreach($venta_clientes as $row) { ?>
                        <tr class="table__tr">
                            <td><?= $row->documento ?></td>                   
                            <td><?= $row->cliente ?></td>
                            <td><?= number_format($row->cantidad_ordenes,2) ?></td>
                            <td><?= number_format($row->subtotal_vendido,2) ?></td>
                            <td><?= number_format($row->ticket_promedio,2) ?></td>
                            <td><?= $row->fechaapro ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
